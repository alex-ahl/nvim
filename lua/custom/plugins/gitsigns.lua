...
return {
  {
    ...
    opts = {
      on_attach = function(bufnr)
        ...
        map('n', '<leader>ghD', gitsigns.preview_hunk_inline, { desc = 'Preview [D]eleted (toggle)' })
        ...
        map('n', '<leader>ghB', function()
          ...
          local base = vim.fn.system('git rev-parse --verify main 2>/dev/null'):find '%w' and 'main' or 'master'
          gitsigns.diffthis(base)
        end, { desc = 'git diff against [B]ase branch' })
        -- Blame lives at the top level: it is line-scoped, not hunk-scoped.
